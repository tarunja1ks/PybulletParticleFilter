...
class Camera(Sensor):
    ...
    def retrieve_data(self, common=True, robot_state=None) -> tuple:
        ...
        Args:
            robot_state: The car's current state. Some sensors
    ...
    def simulate(self, image: tuple) -> None:
        width, height, color, depth, segmentation = image
        # Display the camera image, along with the depth and segmentation image
        # The color image is 480x640x4, the depth image is 480x640x1, and the segmentation image is 480x640x1
        cv2.imshow("Color Image", cv2.cvtColor(color, cv2.COLOR_RGB2BGR))
        cv2.imshow("Depth Image", cv2.applyColorMap(cv2.convertScaleAbs(depth, alpha=0.03), cv2.COLORMAP_JET))
        cv2.waitKey(1)
...
class Lidar(Sensor):
    ...
    def __init__(self,
        ...
        Args:
            robot_id: The unique `robot_id` passed by the object corresponding to the robot.
            configs:
                - lidar_joints: 
                - lidar_angle1: 
                - lidar_angle2: 
...
class IMU(Sensor):
    """
    IMU class to manage the IMU sensor in PyBullet.
    """
    def __init__(self,
                 robot_id: int,
                 configs: dict) -> None:
        ...
        The IMU specifications can be specified in the car's config file.

        Args:
            robot_id: The unique `robot_id` passed by the object corresponding to the robot.
            configs: A dictionary containing IMU configuration parameters.
                - imu_link: The link where the IMU is attached.
        Returns:
            None.
        Raises:
            None.
        ...
        self.IMU_LINK = configs['imu_joints']
        self.car_id = robot_id

    def setup(self) -> None:
        """
        Setup IMU object (a one-time setup).
    ...
    def retrieve_data(self, common=True, robot_state=None) -> dict:
        """
        Retrieve IMU data, including linear acceleration and angular velocity.
        ...
        angular_velocity = np.array(angular_velocity)+ np.random.randn(3) * self.na_std
        ...
        self.prev_linear_velocity = linear_velocity+ np.random.randn(3) * self.ng_std
