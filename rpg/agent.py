    ...
    def __unpack_car_settings(self, car_config: str) -> dict:
        ...
        Args:
            env: A string containig the name of the .yaml
                 file.
        Returns:
            A dictionary containing the car configs that is stored in the .yaml file.
        Raises:
            None.
        """
...
class HuskyKuka:
    """
    A module navigating a Husky robot with a Kuka arm in the sim env.
    """
    def __init__(self, husky_urdf_path: str, kuka_urdf_path: str, husky_config: str, sensors: None, debug=False) -> None:
        """
        Initialize HuskyKuka object.

        Args:
            husky_urdf_path: Path to the Husky URDF file.
            kuka_urdf_path: Path to the Kuka URDF file.
        ...
        self.KUKA_URDF_PATH = kuka_urdf_path
        self.DEBUG = debug
    ...
    def place_robot(self, floor, xy_coord=(0, 0)) -> None:
        """
        Places the Husky robot with Kuka arm in the sim env.
    ...
    def act(self, v,s , f=1000) -> None:
        ...
        if s < 0: # Turn left
            for i, wheel in enumerate(wheels):
                self.wheelVelocities[i] = self.wheelVelocities[i] - self.speed * self.wheelDeltasTurn[i]
        ...
        if v > 0: # Move forward
            for i, wheel in enumerate(wheels):
                self.wheelVelocities[i] = self.wheelVelocities[i] + self.speed * self.wheelDeltasFwd[i]
    ...
    def control_arm_inverse(self, target_pos, target_orn=None, use_null_space=False, use_orientation=False):
        """
        Control the robotic arm to reach a target position and orientation, using inverse kinematics.

        Args:
            target_pos: Target position for the end effector.
            target_orn: Target orientation for the end effector (optional).
            use_null_space: Use null space optimization for inverse kinematics.
            use_orientation: Use orientation in inverse kinematics calculation.
        """
        if use_null_space:
            if use_orientation:
                joint_poses = p.calculateInverseKinematics(self.kuka_id, self.kuka_end_effector_index, target_pos, target_orn, self.ll, self.ul, self.jr, self.rp)
            else:
                joint_poses = p.calculateInverseKinematics(self.kuka_id, self.kuka_end_effector_index, target_pos, lowerLimits=self.ll, upperLimits=self.ul, jointRanges=self.jr, restPoses=self.rp)
        else:
            if use_orientation:
                joint_poses = p.calculateInverseKinematics(self.kuka_id, self.kuka_end_effector_index, target_pos, target_orn, jointDamping=self.jd)
            else:
                joint_poses = p.calculateInverseKinematics(self.kuka_id, self.kuka_end_effector_index, target_pos)

        for i in range(self.num_joints):
            p.setJointMotorControl2(self.kuka_id, i, p.POSITION_CONTROL, targetPosition=joint_poses[i], force=20)

    def control_arm_forward(self, joint_poses):
        """
        Control the robotic arm to reach a target joint configuration.

        Args:
            joint_poses: Target joint configuration.
        """
        for i in range(self.num_joints):
            p.setJointMotorControl2(self.kuka_id, i, p.POSITION_CONTROL, targetPosition=joint_poses[i], force=20)
